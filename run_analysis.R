...
library(dplyr)
data_m_sd  <- select(data,contains("ID_Sub"),contains("Activity"),contains("-mean()"),contains("-std()"))
...
data_melt <- melt(data_m_sd,id=c("ID_Sub","Activity"),measure.vars=c(variables))

data_tidy <- data.frame()
for (i in 1:30){
	dt <- data_melt[data_melt$ID_Sub == i,]
	dtcast <- dcast(dt, Activity ~ variable,mean)
	dtcast_obs <- cbind(i, dtcast)
	data_tidy <- rbind(data_tidy,dtcast_obs)
}
names(data_tidy)[1] <- paste("ID_Sub")
